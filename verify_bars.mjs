// R4.2 верификация: нативные полосы — ХП тикает от урона врага, 0 ошибок
...
const bar = await ev(`(function(){
  const b = document.getElementById("hpBarI")
  const e = document.getElementById("expBarI")
  return JSON.stringify({ hpFound: !!b, hpIsNative: !!(b && b.setBarProgress), expFound: !!e, expIsNative: !!(e && e.setBarProgress) })
})()`, true)
console.log("bars:", bar)
await shot("r42_bars_full")
