...
// буквы (громкость 16×soundVolume — по замеру RMS: step.mp3 — самый тихий сэмпл
// игры, при 2× печать была в ~8 раз тише клика меню и не слышалась за музыкой).
...
function nextLine() {
    ...
    dlgTimer = setInterval(() => {
        ...
        //звук печати — step.mp3 на каждую вторую букву; 16× компенсирует тихий сэмпл
        //(замер RMS в живой странице: при 2× это было 12% громкости клика меню)
        dlgState.shown % 2 === 0 && playback(strike[0].vol,0,0,16*status.settings.soundVolume)
