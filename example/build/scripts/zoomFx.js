...
export function setWorldViewBox(x, y) {
    //A1 (PIXEL_ART_RESEARCH.md §6): снап камеры к целым ФИЗИЧЕСКИМ пикселям экрана.
    //Мир отрисован текстурами с linear-фильтрацией (R2.5); пока левый/верхний край
    //кадра стоит в дробной фазе сэмплинга, размытие «пульсирует» на каждом шаге
    //камеры и мир субпиксельно плавает при скролле. Снап делает фазу стабильной:
    //размытие статичное, движение камеры идёт целыми экранными пикселями.
    //Логика игры не меняется: потребители читают те же x/y (сдвиг < 1 физ. px),
    //дробное движение самих спрайтов (герои/враги) остаётся плавным, как в R2.5.
    const s = (windowSize.wt / 1920) * zoom * Math.min(2, window.devicePixelRatio || 1)
    x = Math.round(x * s) / s
    y = Math.round(y * s) / s
