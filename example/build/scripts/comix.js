...
function comix(fin = 0) {
    ...
    //4-2/4-3 — выбор босса сделан в nextFloor ещё до комикса спуска; V91: Грибу пустоты
    //(id 27) своего арта пока нет — показываем диптих Медузы (решение пользователя))
    if(fin === 1) {
        let finPair = status.meta.page === 4 ?
                      (status.voidBossId === 26 || status.voidBossId === 27 ? ["4-2.png","4-3.png"] : ["4-0.png","4-1.png"]) :
    ...
    //V91: Гриб пустоты — пока тоже диптих Медузы, своего нет)
    if(fin === 2) {
        if(status.meta.page === 4) {
            if(status.voidBossId === 26 || status.voidBossId === 27) {
