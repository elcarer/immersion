...
//(spawnSporeMini: клон класса без тега boss, ХП 1/50 (V113) / урон 1/10+14 / опыт 1/10
//актуальных статов, размер 1/5, скорость полная). При смерти босса споры исчезают
...
//V113 (ТЗ юзера): ещё +4 — хрупкие, но больно бьющие (16-18)
const MINI_DMG_BONUS = 14
...
//статы — от АКТУАЛЬНЫХ параметров босса (после множителей глав): ХП 1/50 maxHp (V113 —
//в 5 раз меньше прежней 1/10), урон 1/10 + MINI_DMG_BONUS (V113: 14), опыт 1/10;
//скорость и зоркость полные, атаки те же (решения пользователя). Структура — как
//у spawnMedusaPiece
function spawnSporeMini(cell) {
    ...
    //V113 (ТЗ юзера): ХП мини в 5 раз меньше прежнего (1/10 → 1/50 от босса, т.е. 20 при 1000)
    let miniHp = Math.max(1, Math.trunc(boss.stats.maxHp / 50))
