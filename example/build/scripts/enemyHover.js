...
function enemyHoverTick() {
    ...
    //V128 (репорт юзера: инфоокна врагов/объектов пропали совсем): с V126 ОС-курсор
    //скрыт ВСЕГДА (единый курсор-спрайт) — гвард по body.style.cursor убил окна
    //навсегда. Скрытность теперь читается с курсора-спрайта (cursorTick прячет его,
    //пока любой герой бежит — та же семантика, что у правила V96)
    if (status.start !== 1 || status.panels !== 0 || status.pause !== 0 || isDragging() ||
        (status.newMouse && status.newMouse.getAttribute("display") === "none")) {
        hovered && closeWindow()
        return
    }
