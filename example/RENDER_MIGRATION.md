...
### R1. Рендер боевых сущностей из компонентов (ядро анти-ряби) — ✅ ВЫПОЛНЕН (2026-09-17)
Фактически сделано: applyStillTexture (общая точка кадра) + anim-ветка applyPosition
пишет в COMPONENTS.animStill (узел не трогает); ecsRenderSync применяет видимость +
позицию + кадр раз в кадр (с ретраем на незагруженные кадры); registerEcs берёт
инициальный кадр из шима (спавны с frame:); roundPixels на мировых спрайтах слоёв 0/1.
Проверка: полный цикл headless (этаж/бой/пули/убийство/смерть/новая игра) без ошибок,
компонент↔узел 1:1, FPS 143–147. Статичные image-сущности и UI не тронуты (R3/R4).
