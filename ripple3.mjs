...
const snap = () => ev(`JSON.stringify((function(){
  const st = window.__ST.status, L = window.__BACKEND.layers, v = window.__BACKEND.cameraVB
  return { hx: Math.round(st.hero.x), floorN: L[0].children.length, objN: L[1].children.length, vbx: Math.round(v.x), vby: Math.round(v.y) }
})())`)
const vbX = async () => JSON.parse(await ev(`JSON.stringify(Math.round(window.__BACKEND.cameraVB.x))`))
...
await S(1000)
console.log("floor:", await snap())
// сразу идём вправо (без простоя — стелс не включится)
...
for (let i = 0; i < 220; i++) {
  await S(55)
  const vx = await vbX()
  if (vx !== vb0) { scrolled = true; break }
}
console.log("scrolled:", scrolled, await snap())
let shot = 0
for (let i = 0; i < 34; i++) {
